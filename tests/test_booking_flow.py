...
"""
Test input cinema params
"""


@pytest.mark.parametrize("inputs,title, row, col", [
    ("INCEPTION 8 10", "INCEPTION", 8, 10),
    ("INCEPTION 2 8 10", "INCEPTION 2", 8, 10),
    ("INCEPTION 2 DIRECTOR\"S CUT 8 10", "INCEPTION 2 DIRECTOR\"S CUT", 8, 10)

])
def test_get_cinema_params(monkeypatch, inputs, title, row, col):
...
@pytest.mark.parametrize("inputs", [
    ("INCEPTION8 10"), ("INCEPTION [8] [10]"),
    ("INCEPTION8 80 9"), ("INCEPTION [10] [50]"),
    ("INCEPTION")
]
)
def test_get_cinema_paras(monkeypatch, inputs):
    ...
    b1 = booking_flow()
    # prompt to enter new one again but not supplied
...
def test_get_booking(capsys, monkeypatch):
...
"""
Test booking flow - add stages of sequential functions incrementally 
1: New booking 
2. change booking 
3. +1 new booking 
